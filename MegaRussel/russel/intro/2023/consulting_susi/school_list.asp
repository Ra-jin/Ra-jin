...
<%
    ...
    If sMode = "M" Then
        ...
        strSql = ""
        strSql = strSql & " SELECT ROW_NO = ROW_NUMBER() OVER(ORDER BY BM_NM), BM_CD, BM_NM FROM MR_BAT_MAJOR a join  WHERE BM_BU_CD = " & sUni_CD & " AND BM_NM LIKE '%" & sMajor_NM & "%'"
    Else
        sMode  = "U" '최초 창이 띄워졌을 때는 sMode 값이 공백, 그래서 기본값으로 U 를 준다.
        sTitle = "대학교"

        strSql = ""
        strSql = strSql & "  SELECT ROW_NO = ROW_NUMBER() OVER(ORDER BY b.BU_NM), b.BU_CD, a.BU_NM FROM MR_BAT_UNIVERSITY_2024 a join MR_BAT_UNIVERSITY b on a.BU_NM = b.BU_NM WHERE BU_NM LIKE '%" & sUni_NM & "%' "
    End if
