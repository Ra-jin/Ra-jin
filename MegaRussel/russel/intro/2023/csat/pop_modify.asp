...
<script>
    ...
    function fncSubmit2() {
        ...
        if (confirm('접수 완료 후, 신청 정보 수정은 11월 15일 오후 2시까지 가능합니다. 수정하시겠습니까?')) {
					...
					<td>
						<div class="btn-wrap">
							<select class="w346" name="SH_EXAM_ADDR" id="SH_EXAM_ADDR">
								...
								<option value="러셀영통" <% If SH_EXAM_ADDR="러셀영통" Then%> selected <% End If %>>러셀 영통</option>
								...
								<option value="러셀울산" <% If SH_EXAM_ADDR="러셀울산" Then%> selected <% End If %> >러셀 울산(2023.12월 개원 예정)</option>
