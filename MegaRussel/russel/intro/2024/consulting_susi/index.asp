...
<%
	Response.Cookies("_CAMPUS_CODE_") = "INTRO"
	...
	If mobileChk = "Y" Then
		urlMain = urlMainMobile & "/intro/intro.asp"
...
<div class="consulting-wrap">
    <div class="visual-area">
        <div class="inner">
            ...
            <!--마감딱지-->
            <div class="ico-end" style="top:617px;right:calc(50% - 300px);"><div>전석마감<br>대기자 접수 중</div></div>
            <!--//마감딱지-->
        </div>
    </div>
...
<script>
	...
	function preMonth() {
		...
		if ($prevActive.length) {
			...
			$('#calendar .r-active').each(function() {
					...
					result.forEach(function(item) {
						...
						<%If bReserv Then%>
							if (dataMyCnt > 0) {
								...
								$('#calendar table tbody').addClass('non-active');
								$('#calendar table tbody:has(div.r-complete)').removeClass();
								var strDate = $('#calendar').find('tbody:not(.non-active) div a[data-cd]').eq(0).attr('data-cd');
								var yearMonth = new Date(strDate);
								var year = yearMonth.getFullYear();
								var month = yearMonth.getMonth() + 1;
		...
		<%Else%>
			...
			if (fnChk()) {
				...
				if (dtlIdx > 0) {
					...
					if (result.length) {
						<%If mobileChk = "Y" Then%>
							//window.open('<%=urlMainMobile%>/intro/2024/consulting_susi/pay.asp?Cole_Cd='+result, 'payPop');
							window.open('<%=urlPay%>?Mode=Register&Cole_Cd='+result+'&Responsive=Y','popup')
						<%Else%>
