...
function fixmenu(){
	...
	$(window).scroll(function(){
		...
		// .cont08 도달하면 실행
		var cont08Offset = $('.cont08').offset().top - 300; 
		...
		// 퀵 fixed 메뉴
		var regularWrapTop = $('.regular-go-wrap').offset().top;
...
function rSlide(){
	...
	$(window).scroll(function(){
		...
		// .cont03 도달하면 실행
		var cont03Offset = $('.cont03').offset().top - 500; 
...
function manageSlide(){
	var mSwiper = new Swiper('.manage-slide',{
		speed: 2000,
		autoplay: {
			delay: 3000,
			disableOnInteraction: false, // 버튼클릭해도 계속 롤링
...
function conSlide(){
	...
	var conSlideNav = new Swiper('.con-slide-navi', {
		slidesPerView: 10,
		freeMode: true,
	...
	var slideCont = new Swiper('.cont-slide', {
		loop: true,
		autoHeight: true,
		speed : 500,
		allowTouchMove: true,
	...
	$(window).scroll(function(){
		...
		// .cont06 도달하면 실행
		var cont06Offset = $('.cont06').offset().top - 500; 
		var cont08Offset = $('.cont08').offset().top;

		if (scroll03  >= cont06Offset && scroll03 < cont08Offset) {
			slideCont.autoplay.start();
		} else if (scroll03 >= cont08Offset) {
			slideCont.autoplay.stop(); // cont08 도달했을 때 정지
		} else {
