$(function(){
	...
	if($('.cont02').hasClass('russel_us')){
		rollSlide02('.s-slide.type02', '.p-type02')
	}
...
function reviewSlideDc(){
	var rSwiper = new Swiper('.review-slide-dc', {
		slidesPerView: 1,
		loop: true,
		allowTouchMove: true,
		autoHeight: true,
	...
	// slide 갯수에 따른 pagination width값 조절
	let slideLeng = $('.review-slide-dc .swiper-wrapper .swiper-slide').length - 2;
	let bulletWidth = 100 / slideLeng
	$(".review-slide-dc.swiper-container-horizontal>.swiper-pagination-bullets .swiper-pagination-bullet").css('width', bulletWidth + "%")
...
/*울산학원 슬라이드*/
function rollSlide02(sSlide, paging){
	var comSlide = new Swiper(sSlide, {
		loop: true,
		centeredSlides: true,
		slidesPerView: "auto",
	...
	$(window).scroll(function(){
		...
		if (scroll >= cont02Top && scroll <= cont03Top) {
			comSlide.autoplay.start();
		} else {
			comSlide.autoplay.stop();
