$(function(){
	...
	if($('.cont06 div').hasClass('time-slide02')){
		timeSlide02();
	}
...
function fixmenu(){
	...
	$(window).scroll(function(){
		...
		//cont07 화살표 애니메이션 실행
		var gtop = $('.cont10').offset().top - 120;
...
function timeSlide(){
	...
	if ($('.cont').hasClass('russel_yt')) {
		// slide 갯수에 따른 pagination width값 조절
		let slideLeng = $('.time-slide .swiper-wrapper .swiper-slide').length - 2;
		let bulletWidth = 100 / slideLeng
		$(".time-slide.swiper-container-horizontal>.swiper-pagination-bullets .swiper-pagination-bullet").css('width', bulletWidth + "%")
...
function timeSlide02 (){

	var timeSwiper = new Swiper('.time-slide02', {
		loop: true,
		spaceBetween: 10,
		//loopAdditionalSlides : 3,
		autoHeight: true,
