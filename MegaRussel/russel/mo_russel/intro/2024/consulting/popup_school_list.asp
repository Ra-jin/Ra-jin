...
<%
    ...
    sType           = VARS("V_TYPE") ' 1: 가군, 2 : 나군, 3: 다군
    ...
    Else
        ...
        strSql = ""
        strSql = strSql & " SELECT ROW_NO = ROW_NUMBER() OVER(ORDER BY BU_NM), BU_CD, BU_NM FROM MR_BAT_UNIVERSITY "
        strSql = strSql & " WHERE BU_NM LIKE '%" & sUni_NM & "%' AND BU_YEAR = 2024 "
        IF sType = "1" THEN
            strSql = strSql & " AND BU_TYPE = '가' "
        ELSEIF sType = "2" THEN
            strSql = strSql & " AND BU_TYPE = '나' "
        ELSEIF sType = "3" THEN
            strSql = strSql & " AND BU_TYPE = '다' "
        END IF
    End if
    ...
    function list_click(mode, code, name) {        
        ...
        if (parent.sKind == "M") {
            ...
            if (mode == "U") {
                ...
                parent.school_search.V_MAJOR_NM.value = name;
