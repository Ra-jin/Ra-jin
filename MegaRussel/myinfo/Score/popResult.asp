...
<%
	...
	sExam_CD         = fncRequest("exam_cd")
	nRmsMemCd		 = fncRequest("mem_cd")
%>
<div class="wrap">
		...
		<div class="content-in">
			...
			<div class="tb-type1">
				<table summary="이 표는 시험정보를 제공하며 시험일자, 시험명, 수험번호, 이름, 원점수총합, 표준점수총합, 백분위총합으로 구성되어있습니다." class="score-info mo-table">
					...
					<tbody><%
						strSQL = ""
						strSQL = strSQL & " SELECT		TM_TRY_CD, dbo.UF_GET_COM_DTL(140, TM_DIV_CD) AS DIV_NM, CONVERT(CHAR(10), TM_REG_DT, 102) AS TRY_DT, TM_TRY_NM, MM_MEM_NM "
						...
						strSQL = strSQL & " INNER JOIN  AMS_MEM_MAS C (NOLOCK) ON B.MS_MEM_CD = C.MM_MEM_CD "
						strSQL = strSQL & " WHERE		A.TM_TRY_CD = '" & sTry_CD & "' "
						strSQL = strSQL & " AND			B.MS_EXAM_CD = '" & sExam_CD & "' "
						strSQL = strSQL & " AND			B.MS_MEM_CD = '" & nRmsMemCd & "' "
						strSQL = strSQL & " GROUP BY	TM_TRY_CD, MS_EXAM_CD, TM_DIV_CD, TM_REG_DT, TM_TRY_NM, MS_MEM_CD, MM_MEM_NM "
						...
						<% else %>
							<tr>
								...
								<td data-label="이름"><%=RS("MM_MEM_NM")%></td>
				...
				<div class="in-horizon">
					<table summary="이 표는 시험정보를 제공하며 시험일자, 시험명, 수험번호, 이름, 원점수총합, 표준점수총합, 백분위총합으로 구성되어있습니다.">
						...
						<tbody><%
							...
							strSQL = strSQL & " 			, (SELECT LANG_RANK FROM (SELECT RANK() OVER (ORDER BY MS_LANG_SPT DESC) AS LANG_RANK, MS_EXAM_CD, MS_MEM_CD FROM AMS_MEM_SCORE (NOLOCK) WHERE MS_TRY_CD = A.MS_TRY_CD AND MS_LANG_CD IS NOT NULL AND MS_LANG_CD <> 0) B WHERE B.MS_EXAM_CD = A.MS_EXAM_CD AND B.MS_MEM_CD = A.MS_MEM_CD) AS LANG_RANK "
							...
							strSQL = strSQL & " 			, (SELECT MATH_RANK FROM (SELECT RANK() OVER (ORDER BY MS_MATH_SPT DESC) AS MATH_RANK, MS_EXAM_CD, MS_MEM_CD FROM AMS_MEM_SCORE (NOLOCK) WHERE MS_TRY_CD = A.MS_TRY_CD AND MS_MATH_CD IS NOT NULL AND MS_MATH_CD <> 0) B WHERE B.MS_EXAM_CD = A.MS_EXAM_CD AND B.MS_MEM_CD = A.MS_MEM_CD) AS MATH_RANK "
							...
							strSQL = strSQL & " 			, (SELECT ENG_RANK FROM (SELECT RANK() OVER (ORDER BY MS_ENG_SPT DESC) AS ENG_RANK, MS_EXAM_CD, MS_MEM_CD FROM AMS_MEM_SCORE (NOLOCK) WHERE MS_TRY_CD = A.MS_TRY_CD AND MS_ENG_CD IS NOT NULL AND MS_ENG_CD <> 0) B WHERE B.MS_EXAM_CD = A.MS_EXAM_CD AND B.MS_MEM_CD = A.MS_MEM_CD) AS ENG_RANK "
							...
							strSQL = strSQL & " 			, (SELECT SEL3_RANK FROM (SELECT RANK() OVER (ORDER BY MS_SEL3_SPT DESC) AS SEL3_RANK, MS_EXAM_CD, MS_MEM_CD FROM AMS_MEM_SCORE (NOLOCK) WHERE MS_TRY_CD = A.MS_TRY_CD AND MS_SEL3_CD IS NOT NULL AND MS_SEL3_CD <> 0) B WHERE B.MS_EXAM_CD = A.MS_EXAM_CD AND B.MS_MEM_CD = A.MS_MEM_CD) AS SEL3_RANK "
							...
							strSQL = strSQL & " 			, (SELECT SEL1_RANK FROM (SELECT RANK() OVER (ORDER BY MS_SEL1_SPT DESC) AS SEL1_RANK, MS_EXAM_CD, MS_MEM_CD FROM AMS_MEM_SCORE (NOLOCK) WHERE MS_TRY_CD = A.MS_TRY_CD AND MS_SEL1_CD IS NOT NULL AND MS_SEL1_CD <> 0) B WHERE B.MS_EXAM_CD = A.MS_EXAM_CD AND B.MS_MEM_CD = A.MS_MEM_CD) AS SEL1_RANK "
							...
							strSQL = strSQL & " 			, (SELECT SEL2_RANK FROM (SELECT RANK() OVER (ORDER BY MS_SEL2_SPT DESC) AS SEL2_RANK, MS_EXAM_CD, MS_MEM_CD FROM AMS_MEM_SCORE (NOLOCK) WHERE MS_TRY_CD = A.MS_TRY_CD AND MS_SEL2_CD IS NOT NULL AND MS_SEL2_CD <> 0) B WHERE B.MS_EXAM_CD = A.MS_EXAM_CD AND B.MS_MEM_CD = A.MS_MEM_CD) AS SEL2_RANK "
							...
							strSQL = strSQL & " 			, (SELECT FOR_RANK FROM (SELECT RANK() OVER (ORDER BY MS_FOR_SPT DESC) AS FOR_RANK, MS_EXAM_CD, MS_MEM_CD FROM AMS_MEM_SCORE (NOLOCK) WHERE MS_TRY_CD = A.MS_TRY_CD AND MS_FOR_CD IS NOT NULL AND MS_FOR_CD <> 0) B WHERE B.MS_EXAM_CD = A.MS_EXAM_CD AND B.MS_MEM_CD = A.MS_MEM_CD) AS FOR_RANK "
							...
							strSQL = strSQL & " AND			A.MS_EXAM_CD = '" & sExam_CD & "' "
