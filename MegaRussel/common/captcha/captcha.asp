<%@LANGUAGE="VBSCRIPT"%>
<!--#include virtual="/common/sha256.asp"-->
<%
...
'#Begin ColorMap
const BmpColorMap = "f6f6f6ec3c491700"
...
dim Bitmap(25,200) '[Height,Width]
CodeLength = 6 'Secure code length (Max:8)
const CodeType = 1 '0[Random numbers], 1[Random chars and numbers], 2[Wordlist]
const CharTracking = 10 'Set the tracking between two characters
const RndTopMargin = True 'Randomize top margin every character
const NoiseEffect = 3 '0[none], 1[sketch], 2[random lines of text color], 3[Random lines of background color over text (Recommed maximum NoiseLine=4)]
...
response.cookies(SessionName) = Base64.encode(SHA256(secureCode))
